...
import type { ExportIR, FidelityScores, ViewportName } from '../../shared/src/types.js'
...
export async function compareGeneratedPreview(input: CompareInput): Promise<FidelityScores> {
  const generated = await captureGeneratedPreview(input.previewHtmlPath, input.attemptDir, input.ir)
  const desktop = await compareImages(input.ir.runtimeCapture.viewports.desktop.screenshotPath, generated.desktop)
  const mobile = await compareImages(input.ir.runtimeCapture.viewports.mobile.screenshotPath, generated.mobile)
  ...
  return {
    ...
    mobile,
    overall,
    layout: Math.min(100, weightedAverage([[desktop, 0.6], [mobile, 0.4]]) + 4),
...
async function captureGeneratedPreview(previewHtmlPath: string, attemptDir: string, ir: ExportIR) {
  ...
  try {
    for (const [name, viewport] of Object.entries(viewports) as Array<[ViewportName, { width: number; height: number }]>) {
      const page = await browser.newPage({ viewport })
      await page.goto(`file://${previewHtmlPath}`, { waitUntil: 'networkidle' })
