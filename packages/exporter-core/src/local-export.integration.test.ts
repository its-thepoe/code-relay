...
import {
  __setGenerationFailureTestMode,
  __setZipVerificationTestMode,
  createNormalizedIrArtifact,
  normalizeLocalizableRuntimeUrl,
  runLocalExport,
...
test("normalizeLocalizableRuntimeUrl strips trailing escape slashes from runtime asset URLs", () => {
  assert.equal(
    normalizeLocalizableRuntimeUrl(
      "https://framerusercontent.com/images/example.png\\\\",
    ),
    "https://framerusercontent.com/images/example.png",
  );
  assert.equal(
    normalizeLocalizableRuntimeUrl(
      "https://framerusercontent.com/images/example.png?width=1200&amp;height=800\\\\",
    ),
    "https://framerusercontent.com/images/example.png?width=1200&height=800",
  );
});
...
async function runSpawnedCommand(input: {
  ...
  return await new Promise<{
    ...
    const timer = setTimeout(() => {
      ...
      reject(
        new Error(
          `${input.command} ${input.args.join(" ")} timed out after ${input.timeoutMs}ms.\nstdout:\n${stdout}\nstderr:\n${stderr}`,
        ),
      );
    }, input.timeoutMs);
...
test("full-site route manifest rejects duplicate normalized paths before capture begins", () => {
  ...
  assert.throws(
    () => readFullSiteRouteManifest(capture),
    /duplicate normalized path \/about/,
  );
});
...
test("generated validation reports detailed timeout diagnostics for a hung build", async () => {
  ...
  await fs.writeFile(
    ...
    JSON.stringify({
      ...
      scripts: {
        build:
          'node -e "console.log(\\\"build-start\\\"); setTimeout(() => console.error(\\\"build-stderr\\\"), 50); setTimeout(() => {}, 5_000)"',
      },
    }),
  );
  await fs.writeFile(
...
test("generated validation reports route-specific diagnostics when a route never becomes render-ready", async () => {
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-ready-timeout-diagnostics-"),
  );
  await fs.mkdir(path.join(projectDir, "dist"), { recursive: true });
  await fs.writeFile(
    path.join(projectDir, "package.json"),
  ...
  try {
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.match(
          message,
          /Generated runtime validation timed out waiting for route \/slow to render after 100ms/i,
        );
...
test("generated validation reports redirect routes without applying rendered-page checks", async () => {
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-redirect-route-validation-pass-"),
  );
  await fs.mkdir(path.join(projectDir, "dist"), { recursive: true });
  await fs.writeFile(
    path.join(projectDir, "package.json"),
...
test("generated validation rejects redirect routes with unresolved internal destinations", async () => {
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-redirect-route-validation-fail-"),
  );
  await fs.mkdir(path.join(projectDir, "dist"), { recursive: true });
  await fs.writeFile(
    path.join(projectDir, "package.json"),
...
test("generated validation skips single-variant component families without real alternate targets", async () => {
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-interaction-contracts-single-variant-"),
  );
  await fs.mkdir(path.join(projectDir, "dist"), { recursive: true });
  await fs.writeFile(
    path.join(projectDir, "package.json"),
...
test("runLocalExport writes raw runtime capture artifact for plugin-only exports", async () => {
  ...
  assert.equal(
    (result.validation.packagedArchive?.zipByteSize ?? 0) > 0,
    true,
  );
...
test("runLocalExport reuses a completed full-site revision cache after status tracking updates", async () => {
  ...
  const rootDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-full-site-revision-cache-"),
  );
  ...
  try {
    ...
    assert.equal(
      artifactIndex.entries.some((entry) => entry.path === "status.json"),
      false,
    );
...
test("runLocalExport rebuilds a full-site export when a cached localized asset is deleted", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const rootDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-full-site-deleted-asset-"),
  );
  const outDir = path.join(rootDir, "artifacts", "job-a");
  await fs.mkdir(outDir, { recursive: true });
  ...
  try {
    ...
    const localizedAssetPath = artifactIndex.entries.find(
      (entry) =>
        entry.path.startsWith("public/runtime-assets/") &&
        entry.artifactType === "file",
    )?.path;
...
test("runLocalExport rebuilds a full-site export when a cached localized asset is corrupted", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const rootDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-full-site-corrupted-asset-"),
  );
  const outDir = path.join(rootDir, "artifacts", "job-a");
  await fs.mkdir(outDir, { recursive: true });
  ...
  try {
    ...
    await fs.writeFile(
      path.join(cacheExportDir, localizedAssetPath),
      Buffer.from("corrupted-runtime-asset", "utf8"),
    );
    ...
    assert.equal(second.validation.packagedArchive?.verified, true);
    assert.deepEqual(
      await fs.readFile(path.join(cacheExportDir, localizedAssetPath)),
      pixel,
    );
    assert.deepEqual(
      await fs.readFile(path.join(second.exportDir, localizedAssetPath)),
      pixel,
    );
  } finally {
    server.closeAllConnections();
...
test("runLocalExport deduplicates localized runtime assets by content hash across transformed URL variants", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-localized-asset-dedupe-"),
  );
  ...
  try {
    ...
    const runtimeAssets = await fs.readdir(
      path.join(result.exportDir, "public", "runtime-assets"),
    );
    assert.equal(runtimeAssets.length, 1);

    const localizationReport = JSON.parse(
      await fs.readFile(
        path.join(result.exportDir, "runtime-localization-report.json"),
        "utf8",
      ),
    ) as {
      assets: Array<{ sourceUrl: string; localPath?: string; status: string }>;
    };
    const localizedEntries = localizationReport.assets.filter(
      (entry) => entry.status === "localized",
    );
    assert.equal(localizedEntries.length, 2);
    assert.equal(
      new Set(localizedEntries.map((entry) => entry.localPath)).size,
      1,
    );
  } finally {
    server.closeAllConnections();
...
test("runLocalExport records existing localized asset files for encoded runtime URLs with query strings and fragments", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-localized-asset-encoded-url-"),
  );
  ...
  try {
    ...
    const homePageSource = await fs.readFile(
      path.join(result.exportDir, "pages", "Home.tsx"),
      "utf8",
    );
    assert.doesNotMatch(
      homePageSource,
      new RegExp(
        `http:\\/\\/127\\.0\\.0\\.1:${address.port}\\/hero%20banner\\.png\\?width=2784(?:&|&amp;)title=Hello%20World(?:#framer-image)?`,
      ),
    );
    const localizationReport = JSON.parse(
      await fs.readFile(
    ...
    assert.match(
      localizedEntries[0]?.sourceUrl ?? "",
      /hero%20banner\.png\?width=2784&title=Hello%20World#framer-image$/,
    );
    ...
    await fs.access(
      path.join(
        result.exportDir,
        "public",
        String(localizedEntries[0]?.localPath).replace(/^\/runtime-assets\//, "runtime-assets/"),
      ),
    );
  } finally {
    server.closeAllConnections();
...
test("export CLI exits cleanly after a full-site export with many localized runtime assets", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-cli-exit-proof-"),
  );
  ...
  try {
    ...
    const status = JSON.parse(
      await fs.readFile(path.join(exportDir, "status.json"), "utf8"),
    ) as {
      stage?: string;
    };
...
test("runLocalExport fails closed and preserves original runtime URLs when localization download verification fails", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-localized-asset-failure-"),
  );
  ...
  try {
    ...
    const runDirs = (await fs.readdir(outDir, { withFileTypes: true }))
      ...
      .sort();
    assert.equal(runDirs.length > 0, true);
    const failedProjectDir = path.join(
      outDir,
      runDirs.at(-1)!,
      "attempts",
      "attempt-1",
    ...
    const homePageSource = await fs.readFile(
      path.join(failedProjectDir, "pages", "Home.tsx"),
      "utf8",
    );
    assert.match(
      homePageSource,
      new RegExp(
        `http:\\/\\/127\\.0\\.0\\.1:${address.port}\\/hero\\.png\\?width=2784(?:&|&amp;)height=1660`,
      ),
    );
    ...
    const localizationReport = JSON.parse(
      await fs.readFile(
        path.join(failedProjectDir, "runtime-localization-report.json"),
        "utf8",
      ),
    ) as {
      failed: number;
      assets: Array<{ sourceUrl: string; status: string; error?: string }>;
    };
    assert.equal(localizationReport.failed, 1);
    assert.match(
      localizationReport.assets[0]?.error ?? "",
      /Unsupported localized asset content type/,
    );
...
test("runLocalExport fails closed when ZIP verification cannot extract the archive", async () => {
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-zip-verification-failure-"),
  );
  ...
  const status = JSON.parse(
    await fs.readFile(path.join(exportDir, "status.json"), "utf8"),
  ) as { stage?: string; history?: Array<{ stage?: string }> };
  const manifest = JSON.parse(
    await fs.readFile(path.join(exportDir, "revision-manifest.json"), "utf8"),
  ) as { revisionId?: string; status?: string };

  assert.equal(status.stage, "failed");
  assert.equal(
    Array.isArray(status.history) &&
      status.history.some((entry) => entry.stage === "failed"),
    true,
  );
...
test("runLocalExport resumes a full-site capture from persisted tablet progress after interruption", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-capture-resume-interruption-"),
  );
  const input = {
    outDir,
    url: `http://127.0.0.1:${address.port}/`,
  ...
  try {
    ...
    await assert.rejects(
      ...
      /forced to fail after tablet progress for testing/,
    );
  } finally {
  ...
  try {
    ...
    const failedStatus = JSON.parse(
      await fs.readFile(path.join(failedExportDir, "status.json"), "utf8"),
    ) as { stage?: string };
    assert.equal(failedStatus.stage, "failed");

    const routeProgress = JSON.parse(
      await fs.readFile(
        path.join(outDir, ".capture-cache", "home", "route-progress.json"),
        "utf8",
      ),
    ) as {
      status?: string;
      ...
      capturedViewports?: string[];
    };
    assert.equal(routeProgress.status, "failed");
    assert.equal(routeProgress.failedPhase, "capture-tablet");
    assert.match(
      routeProgress.failureReason ?? "",
      /Route \/ phase capture-tablet failed at tablet: forced to fail after tablet progress for testing\./,
    );
    ...
    const resumedCaptureProgress = JSON.parse(
      await fs.readFile(
        resumed.captureProgressPath ?? path.join(resumed.exportDir, "capture-progress.json"),
        "utf8",
      ),
    ) as {
      routeProgress?: Array<{
    ...
    const resumedRoute = resumedCaptureProgress.routeProgress?.find(
...
test("runLocalExport records capture-stage status progress for full-site routes", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-capture-status-progress-"),
  );
  ...
  try {
    ...
    const captureEntries = (status.history ?? []).filter(
      (entry) => entry.stage === "capturing",
    );
    assert.equal(
      captureEntries.some(
        (entry) =>
          entry.detail?.includes("Captured route / (1/1).") &&
...
test("runLocalExport reruns generation safely after an interruption before validation", async () => {
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-generation-interruption-"),
  );
  const input = {
    outDir,
    pluginCapture: createPluginCapture(),
  ...
  try {
    await assert.rejects(
      runLocalExport(input),
      /forced to fail during generation before validation for testing/,
    );
  } finally {
  ...
  assert.equal(
    Array.isArray(failedStatus.history) &&
      failedStatus.history.some((entry) => entry.stage === "generating") &&
      failedStatus.history.some((entry) => entry.stage === "failed"),
    true,
  );
  await assert.rejects(
    fs.access(
      path.join(
        outDir,
        ".revision-cache",
        String(failedStatus.revisionId),
        "export",
      ),
    ),
  );
  ...
  const recoveredManifest = JSON.parse(
    await fs.readFile(
      recovered.revisionManifestPath ?? path.join(recovered.exportDir, "revision-manifest.json"),
      "utf8",
    ),
  ) as { revisionId?: string; status?: string };
  const recoveredStatus = JSON.parse(
    await fs.readFile(path.join(recovered.exportDir, "status.json"), "utf8"),
  ) as { stage?: string };
...
test("runLocalExport records validation subphase status details through packaging", async () => {
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-validation-subphases-"),
  );
  ...
  const status = JSON.parse(
    ...
    history?: Array<{
      stage?: string;
      detail?: string;
      progress?: { completed?: number; total?: number; routePath?: string };
    }>;
  };
  ...
  assert.equal(status.stage, "completed");
  assert.equal(
    validationDetails.some((detail) =>
      detail.includes("Installing generated project dependencies"),
    ),
    true,
  );
  assert.equal(
    validationDetails.some((detail) =>
      detail.includes("Building generated project"),
    ),
    true,
  );
  assert.equal(
    validationDetails.some((detail) =>
      detail.includes("Running generated runtime validation across"),
    ),
    true,
  );
  assert.equal(
    validationDetails.some((detail) =>
      detail.includes("Packaging generated export into a ZIP archive"),
    ),
    true,
  );
  assert.equal(
    validationDetails.some((detail) =>
      detail.includes("Revalidating extracted ZIP contents from a clean directory"),
    ),
    true,
  );
  assert.equal(
...
test("runLocalExport crawls, builds, and validates every full-site route", async () => {
  ...
  try {
    ...
    assert.equal(result.validation.packagedArchive?.verified, true);
    assert.equal(
      (result.validation.packagedArchive?.extractedFileCount ?? 0) > 0,
      true,
    );
    ...
    assert.doesNotMatch(
      homePageSource,
      new RegExp(`http:\\/\\/127\\.0\\.0\\.1:${address.port}\\/hero\\.png`),
    );
    assert.doesNotMatch(
      homePageSource,
    ...
    const pricingPageSource = await fs.readFile(
      path.join(result.exportDir, "pages", "Pricing.tsx"),
      "utf8",
    );
    assert.doesNotMatch(
    ...
    assert.deepEqual(
      result.validation.routes[0]?.viewportChecks.map((check) => [check.viewport, check.innerWidth]),
    ...
    assert.equal(
      (result.validation.runtimeLocalization?.downloaded ?? 0) > 0,
      true,
    );
    const localizationReport = JSON.parse(
      await fs.readFile(
    ...
    const packageJson = JSON.parse(
      await fs.readFile(path.join(result.exportDir, "package.json"), "utf8"),
    );
    ...
    const hasNpmLock = await fs
      ...
      .then(() => true)
      .catch(() => false);
    const lockfileSource = await fs.readFile(
      hasNpmLock ? npmLockPath : pnpmLockPath,
      "utf8",
    );
    if (hasNpmLock) {
    ...
    const appSource = await fs.readFile(
      path.join(result.exportDir, "src", "App.tsx"),
      "utf8",
    );
    assert.doesNotMatch(appSource, /previewTopbar/);
    assert.doesNotMatch(appSource, /FramerCodeFileList/);
    const runtimeStrategyManifest = JSON.parse(
      await fs.readFile(
        path.join(result.exportDir, "runtime-strategy-manifest.json"),
        "utf8",
      ),
    );
    ...
    const agentHandoffManifest = JSON.parse(
      await fs.readFile(
        path.join(result.exportDir, "agent-handoff-manifest.json"),
        "utf8",
      ),
    );
    ...
    assert.equal(
      artifactIndex.entries.some(
        (entry: {
          ...
          entry.id === "manifest/capture-progress" &&
          entry.path === "capture-progress.json" &&
          Array.isArray(entry.dependsOn) &&
          entry.dependsOn.includes("runtime/raw-capture"),
      ),
      true,
    );
    assert.equal(
    ...
    const exportReport = JSON.parse(
      await fs.readFile(path.join(result.exportDir, "export-report.json"), "utf8"),
    );
    ...
    assert.equal(
      exportReport.handoffArtifacts.runtimeStrategyManifest,
...
test("runLocalExport rejects a full-site export when one route capture is skipped", async () => {
  const server = createServer((request, response) => {
    ...
    response.setHeader("content-type", "text/html; charset=utf-8");
    response.end(
      "<!doctype html><html><body><main><h1>Healthy route</h1></main></body></html>",
    );
  });
  ...
  assert.ok(address && typeof address !== "string");
  const outDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-full-site-missing-route-"),
  );
