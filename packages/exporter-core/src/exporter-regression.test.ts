...
import {
  ...
  __setInteractionStyleCollectionTestMode,
  __setViewportDriftTestMode,
...
test("Plan 003 blocks full-site exports when a page root is truncated", () => {
  const diagnostics: PluginCaptureDiagnostics = {
...
test("Plan 003 allows component-only truncation", () => {
  ...
  assert.deepEqual(
    ir.warnings.find((warning) => warning.type === "capture_truncated"),
    {
...
test("interaction replay screenshots survive blocked fonts", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-replay-blocked-fonts-"),
  );
...
test("full-site capture validation requires every route and all four exact viewports", async () => {
  ...
  const widths = { desktop: 1440, laptop: 1280, tablet: 768, mobile: 390 } as const;
  const heights = { desktop: 900, laptop: 900, tablet: 1024, mobile: 844 } as const;
  const viewportNames = Object.keys(widths) as Array<keyof typeof widths>;
  ...
  await Promise.all(
    viewportNames.map((name) => fs.writeFile(viewports[name].screenshotPath, "png")),
  );
  const capture = {
    ...createRuntimeCapture(),
    mode: "page" as const,
  ...
  } as unknown as RuntimeCapture;

  try {
    await validateFullSiteCapture({ routes: [{ path: "/" }], capture });
    await assert.rejects(
      validateFullSiteCapture({ routes: [{ path: "/" }, { path: "/missing" }], capture }),
      /route \/missing was not captured/,
    );
    ...
    await assert.rejects(
      validateFullSiteCapture({ routes: [{ path: "/missing" }], capture: failedRouteCapture }),
      /route \/missing was not captured \(Route capture exceeded 3 minutes: \/missing\)/,
    );
    ...
    await assert.rejects(
      validateFullSiteCapture({ routes: [{ path: "/" }], capture: invalidCapture }),
      /duplicate observed viewport width|width mismatch/,
    );
  } finally {
    await fs.rm(workDir, { recursive: true, force: true });
  }
...
test("full-site route cache recaptures only a corrupted cached viewport screenshot", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-cache-corrupt-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  ...
  try {
    ...
    const mobileScreenshotPath =
      first.routeCaptures?.[0]?.viewports.mobile?.screenshotPath;
    ...
    const reusedViewports = (
      second.routeCaptures?.[0]?.captureDiagnostics?.phaseHistory ?? []
    )
      .filter((phase) => phase.status === "reused")
      .map((phase) => phase.viewportName)
      .filter(
        (viewportName): viewportName is NonNullable<typeof viewportName> =>
          Boolean(viewportName),
      );
    const completedViewports = (
      ...
      .filter(
        (phase) =>
          phase.status === "completed" && phase.phase.startsWith("capture-"),
      )
      .map((phase) => phase.viewportName)
      .filter(
        (viewportName): viewportName is NonNullable<typeof viewportName> =>
    ...
    assert.equal(
      second.routeCaptures?.[0]?.captureDiagnostics?.viewportValidation?.mobile
        ?.valid,
      true,
    );
  } finally {
...
test("full-site route cache ignores stale source URLs", async () => {
  ...
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-cache-stale-source-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  ...
  try {
    ...
    assert.equal(
      second.captureDiagnostics?.routeProgress?.some(
        (entry) => entry.reusedFromCache,
      ),
      false,
    );
  } finally {
...
test("full-site route capture resumes a missing viewport from route progress", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-resume-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  ...
  try {
    ...
    assert.ok(partial);
    delete (partial as any).viewports.mobile;
    const partialDiagnostics = partial!.captureDiagnostics as any;
    ...
    partialDiagnostics.breakpointsCaptured = (
      partial!.captureDiagnostics!.breakpointsCaptured ?? []
    ).filter((viewportName) => viewportName !== "mobile");

    await fs.mkdir(path.join(cacheDir, "home"), { recursive: true });
    await fs.writeFile(
      path.join(cacheDir, "home", "route-progress.json"),
      `${JSON.stringify(
        {
          schemaVersion: 3,
          sourceUrl: originUrl,
    ...
    assert.equal(requests, requestsAfterFirstRun + 1);
    assert.equal(
      second.routeCaptures?.[0]?.captureDiagnostics?.breakpointsCaptured.includes(
        "mobile",
      ),
      true,
    );
...
test("full-site route capture retries a transient navigation failure and still completes the route", async () => {
  ...
  const server = createServer((request, response) => {
    ...
    response.setHeader("content-type", "text/html; charset=utf-8");
    response.end(
      "<main><h1>Recovered route</h1><p>Transient network errors should retry.</p></main>",
    );
  });
  await new Promise<void>((resolve) =>
    server.listen(0, "127.0.0.1", resolve),
  ...
  assert.ok(address && typeof address !== "string");
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-transient-retry-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  ...
  try {
    ...
    assert.equal(
      capture.captureDiagnostics?.routeProgress?.some(
        (entry) =>
          entry.routePath === "/" &&
          entry.status === "fresh" &&
          entry.reusedFromProgress !== true,
      ),
      true,
    );
...
test("full-site capture does not abort early on three route-level DNS failures when the origin probe still succeeds", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-network-probe-recovery-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  const workDir = path.join(parentDir, "work");
  const originUrl = `http://127.0.0.1:${address.port}/`;
  ...
  try {
    ...
    assert.equal(
      capture.captureDiagnostics?.routeFailures?.length,
      3,
    );
    await assert.doesNotReject(() =>
      validateFullSiteCapture({
...
test("full-site capture keeps required desktop evidence when interaction-style collection is slow", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-slow-interaction-style-"),
  );
  const originUrl = `http://127.0.0.1:${address.port}/`;
  ...
  try {
    ...
    assert.equal(
      capture.captureDiagnostics?.routeProgress?.[0]?.warningCount,
      1,
    );
    await assert.doesNotReject(() =>
      validateFullSiteCapture({
...
test("full-site route progress ignores stale source URLs", async () => {
  ...
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-progress-stale-source-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  ...
  try {
    ...
    assert.equal(
      second.captureDiagnostics?.routeProgress?.some(
        (entry) => entry.reusedFromProgress,
      ),
      false,
    );
  } finally {
...
test("full-site route progress ignores legacy entries missing the compatibility key", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-progress-legacy-compat-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  const originUrl = `http://127.0.0.1:${address.port}/`;
...
test("full-site route progress does not trust complete snapshots missing required phase evidence", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const parentDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-route-progress-missing-phase-"),
  );
  const cacheDir = path.join(parentDir, "cache");
  const originUrl = `http://127.0.0.1:${address.port}/`;
  ...
  try {
      ...
      `${JSON.stringify(
        {
          ...
          routePath: "/",
          status: "complete",
          phases: (firstCapture!.captureDiagnostics?.phaseHistory ?? []).filter(
            (phase) =>
              !(
                phase.phase === "capture-mobile" &&
    ...
    assert.equal(
      second.captureDiagnostics?.routeProgress?.some(
        (entry) => entry.status === "retried" && entry.reusedFromProgress,
      ),
      true,
    );
  } finally {
...
test("late external redirects use a mobile-safe placeholder on Trusted Types protected documents", async () => {
  ...
  const source = createServer((_request, response) => {
    response.setHeader("content-type", "text/html; charset=utf-8");
    response.end(
      `<main>Redirecting</main><script>setTimeout(() => location.href = ${JSON.stringify(targetUrl)}, 50)</script>`,
    );
  });
  await new Promise<void>((resolve) =>
  ...
  try {
    ...
    assert.equal(
      capture.routeCaptures?.[0]?.redirectTo,
      targetUrl,
    );
    assert.equal(
...
test("internal delayed JavaScript redirects are captured as redirect routes", async () => {
  const source = createServer((_request, response) => {
    response.setHeader("content-type", "text/html; charset=utf-8");
    response.end(
      `<!doctype html><html><body><main>Redirecting internally</main><script>setTimeout(() => location.href = '/learn?from=home#cta', 50)</script></body></html>`,
    );
  });
  await new Promise<void>((resolve) =>
    source.listen(0, "127.0.0.1", resolve),
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-internal-js-redirect-"),
  );
...
test("explicit external redirect metadata short-circuits capture into a redirect placeholder", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-explicit-external-redirect-"),
  );
  ...
  try {
    ...
    assert.equal(
      routeCapture?.nodes.some((node) =>
        node.text?.startsWith("Continue to https://x.com/coderelay"),
      ),
      true,
    );
    assert.equal(
      routeCapture?.captureDiagnostics?.phaseHistory?.some(
        (phase) =>
          phase.phase === "stabilize" &&
          phase.status === "skipped" &&
          phase.detail === "redirect-placeholder",
      ),
      true,
    );
  } finally {
...
test("declared redirect metadata alone does not reinterpret an explicit page capture as a redirect", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-explicit-page-route-kind-"),
  );
...
test("redirects that happen during DOM extraction retry and resolve to a redirect placeholder", async () => {
  ...
  assert.ok(sourceAddress && typeof sourceAddress !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-redirect-during-extraction-"),
  );
  ...
  try {
    ...
    assert.equal(
      capture.routeCaptures?.[0]?.nodes.some((node) =>
        node.text?.startsWith("Continue to "),
      ),
      true,
    );
  } finally {
...
test("capture tolerates never-loading fonts and still records viewport evidence", async () => {
  const fontRequests: Array<import("node:http").ServerResponse> = [];
  const server = createServer((request, response) => {
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-never-loading-font-"),
  );
  ...
  try {
    ...
    assert.equal(
      capture.routeCaptures?.[0]?.captureDiagnostics?.breakpointsCaptured.includes(
        "mobile",
      ),
      true,
    );
    assert.equal(
      capture.routeCaptures?.[0]?.captureDiagnostics?.fontReadiness?.desktop,
      false,
    );
  } finally {
    server.closeAllConnections();
...
test("capture retries a screenshot once when viewport dimensions drift mid-capture", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-viewport-drift-retry-"),
  );
  ...
  try {
    ...
    const validation =
      capture.routeCaptures?.[0]?.captureDiagnostics?.viewportValidation?.desktop;
...
test("capture fails with a precise viewport error when dimensions keep drifting", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-viewport-drift-fail-"),
  );
  ...
  try {
    ...
    assert.match(
      validation?.reason ?? "",
      /viewport-drift|innerWidth-mismatch|screenshotWidth-mismatch/,
    );
    await assert.rejects(
      validateFullSiteCapture({
        routes: [{ path: "/" }],
...
test("capture tolerates body replacement during hydration", async () => {
  ...
  assert.ok(address && typeof address !== "string");
  const workDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-body-replacement-"),
  );
  ...
  try {
    ...
    assert.equal(
      capture.routeCaptures?.[0]?.nodes.some((node) =>
        node.text?.includes("Hydrated page"),
      ),
      true,
    );
  } finally {
...
test("runtime-first full-site redirect captures preserve redirect metadata through IR and route manifest", async () => {
  ...
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-runtime-redirect-roundtrip-"),
  );
  ...
  const route = routeManifest.find(
    (entry: { path?: string }) => entry.path === "/linkedin",
  );
...
test("generated route metadata preserves explicit page kind instead of re-inferring a redirect", async () => {
  ...
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-page-route-kind-wins-"),
  );
  const pageIr = {
    ...base,
...
test("full-site page generation keeps inline component-family imports on page components", async () => {
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-family-page-import-"),
  );
  const base = buildIntermediateRepresentation({
    url: "https://example.com",
    name: "FamilyPage",
  ...
  const aboutPage = await fs.readFile(
    path.join(projectDir, "pages", "About.tsx"),
    "utf8",
  );
...
test("generateNextProject writes non-empty css and imports it from the component", async () => {
  ...
  assert.match(
    framerDataRegistry,
    /getFramerRegisteredComponent\([\s\S]*name: string,[\s\S]*\): FramerComponentRegistryEntry \| undefined/,
  );
  ...
  assert.match(
    framerDataComponentRuntime,
    /Object\.entries\(framerComponentRegistry\) as Array<\s+\[string, FramerComponentRegistryEntry\]\s+>/s,
  );
...
test("generateNextProject keeps link-field label inference type-safe in cms sections", async () => {
  ...
  const projectDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-cms-link-fixture-"),
  );
  ...
  const framerDataCmsRuntime = await fs.readFile(
    path.join(projectDir, "src", "framer-data", "cms-runtime.tsx"),
    "utf8",
  );
  const framerDataCmsSections = await fs.readFile(
    path.join(projectDir, "src", "framer-data", "cms-sections.tsx"),
    "utf8",
  );
