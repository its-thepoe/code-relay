...
test("buildAttemptPlan produces targeted diagnosis and patch operations", () => {
  ...
  assert.ok(
    plan.diagnosis.some((message) =>
      message.includes("without non-default styles"),
    ),
  );
});
...
test("buildAttemptPlan scopes responsive patch targets to smaller-breakpoint diagnostics", () => {
  ...
  assert.deepEqual(
    plan.patchPropertyHints?.promote_viewport_overrides?.["tablet-wrapper"],
    {
      tablet: ["paddingTop", "gap"],
    },
  );
  assert.deepEqual(
    plan.patchPropertyHints?.promote_viewport_overrides?.["mobile-body"],
    {
      mobile: ["fontSize", "lineHeight"],
    },
  );
  assert.equal(
    plan.patchTargets?.promote_viewport_overrides?.includes("desktop-heading"),
    false,
  );
});
...
test("applyAttemptPlan limits motion preservation to hinted interactive properties", () => {
  ...
  assert.equal(
    "color" in (next.ir.exportTree?.[0]?.interactionStyles?.hover ?? {}),
    false,
  );
});
