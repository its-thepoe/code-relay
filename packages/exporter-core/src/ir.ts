...
import type { ExportIR, ExportWarning, NodeMatch, PluginCanvasCapture, RuntimeCapture, RuntimeNode } from '../../shared/src/types.js'
...
function pickContentNodes(nodes: RuntimeNode[]) {
  const usefulTags = new Set(['h1', 'h2', 'h3', 'p', 'a', 'button', 'img', 'span', 'li'])
...
function splitLongPageIntoSections(nodes: RuntimeNode[], matches: NodeMatch[]) {
  ...
  for (const node of sorted) {
    ...
    const startsNewByGap = previous ? node.rect.y - (previous.rect.y + previous.rect.height) > 420 : false
...
function inferSectionName(nodes: RuntimeNode[], index: number) {
  return nodes.find((node) => node.tag === 'h1' || node.tag === 'h2' || node.tag === 'h3')?.text?.slice(0, 48) ?? `Section ${index + 1}`
}

function inferSectionKind(nodes: RuntimeNode[], index: number): 'hero' | 'content' | 'media-grid' {
...
function sectionConfidence(nodes: RuntimeNode[], matches: NodeMatch[]) {
  ...
  return Number((relevant.reduce((sum, match) => sum + match.confidence, 0) / relevant.length).toFixed(3))
}

function inferSemanticType(nodes: RuntimeNode[]): ExportIR['component']['semanticType'] {
