...
test("createCompletedOutcomeCopy returns partial-safe copy for every surface", () => {
  ...
  assert.equal(
    createCompletedOutcomeCopy({ report, surface: "plugin-card" }),
    "Export complete with partial source-aware evidence. Review the report before trusting it as a reusable baseline.",
  );
  assert.equal(
    createCompletedOutcomeCopy({ report, surface: "job-banner" }),
    "Export completed with partial source-aware evidence. Review warnings before trusting the output as a full-fidelity baseline.",
  );
  assert.equal(
    createCompletedOutcomeCopy({ report, surface: "worker-log" }),
    "completed with partial source-aware evidence",
  );
});
