...
export function generateViteConfig(entries: Array<{ name: string; html: string }>): string {
  const input = entries
    ...
    .join(",\n");
  const routeNames = entries.map((entry) => JSON.stringify(`/${entry.name}`)).join(",\n");
...
${routeNames}
  ...
  return {
    name: "ditto-route-redirects",
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        const url = req.url ? new URL(req.url, "http://localhost") : null;
        if (!url || !routes.has(url.pathname) || url.pathname.endsWith("/")) return next();
        res.statusCode = 302;
