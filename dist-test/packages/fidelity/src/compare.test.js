...
import { aggregateComparisonDiagnostics, collectNodePropertyDiffs, compareGeneratedPreview, inspectGeneratedPreviewNodes, scorePreviewValidation, scorePreviewValidationByViewport, } from "./compare.js";
function createIr() {
    return {
...
test("compareGeneratedPreview scores captured motion when screenshots are unavailable", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-compare-motion-"));
...
test("compareGeneratedPreview labels screenshot-backed fidelity when screenshots are available", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-compare-screenshot-"));
    const previewPath = path.join(attemptDir, "preview.html");
    await fs.writeFile(previewPath, "<!doctype html><html><body style=\"margin:0;background:#fff\"></body></html>\n");
...
test("compareGeneratedPreview records the underlying preview capture error when screenshots cannot be generated", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-compare-preview-error-"));
    for (const [viewport, size] of Object.entries({
        desktop: { width: 1440, height: 900 },
        laptop: { width: 1280, height: 900 },
        tablet: { width: 768, height: 1024 },
    ...
    assert.equal(fidelity.evidence.mode, "heuristic");
    assert.match(fidelity.evidence.reason, /Generated preview capture error:/);
    const captureError = JSON.parse(await fs.readFile(path.join(attemptDir, "generated-preview-capture-error.json"), "utf8"));
...
test("compareGeneratedPreview stays screenshot-backed when preview fonts are slow", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-compare-preview-slow-fonts-"));
    ...
    try {
      ...
      <html>
        ...
        await assert.rejects(fs.access(path.join(attemptDir, "generated-preview-capture-error.json")));
    }
    finally {
...
test("compareGeneratedPreview gives styled previews a higher fallback score than unstyled previews", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-compare-preview-score-"));
    ...
    await fs.writeFile(styledPreviewPath, `<!doctype html>
<html>
  <body>
    <h1
      class="nodeHeading"
...
`);
    await fs.writeFile(unstyledPreviewPath, `<!doctype html><html><body><h1 class="nodeHeading">Animated heading</h1></body></html>\n`);
    ...
    assert.equal((styled.previewValidation?.summary.nodesWithNonDefaultStyles ?? 0) >
        (unstyled.previewValidation?.summary.nodesWithNonDefaultStyles ?? 0), true);
...
test("inspectGeneratedPreviewNodes captures hover and focus interaction deltas", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-inspect-preview-interactions-"));
...
`);
    ...
    assert.deepEqual(inspected[0]?.interactionStyles?.hover?.transform, "matrix(1, 0, 0, 1, 0, -2)");
...
test("compareGeneratedPreview validates hover and focus interaction motion", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-compare-interactive-motion-"));
...
`);
    ...
    assert.equal(result.diagnostics?.nodes[0]?.propertyDiffs.some((diff) => diff.property === "hover.transform"), false);
});
test("compareGeneratedPreview uses viewport-specific preview validation for fallback breakpoint scores", async () => {
    const attemptDir = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-compare-responsive-preview-score-"));
    const responsivePreviewPath = path.join(attemptDir, "responsive-preview.html");
    const desktopOnlyPreviewPath = path.join(attemptDir, "desktop-only-preview.html");
    await fs.writeFile(responsivePreviewPath, `<!doctype html>
<html>
  <head>
    <style>
...
`);
    await fs.writeFile(desktopOnlyPreviewPath, `<!doctype html>
<html>
  <head>
    <style>
      .nodeHeading { color: rgb(17, 17, 17); font-size: 40px; line-height: 48px; }
...
`);
    ...
    assert.equal((responsive.fidelity.tablet ?? 0) > (desktopOnly.fidelity.tablet ?? 0), true);
    assert.equal((responsive.fidelity.mobile ?? 0) > (desktopOnly.fidelity.mobile ?? 0), true);
});
...
test("collectNodePropertyDiffs compares broader layout and surface properties", () => {
    ...
    assert.equal(compared.propertyDiffs.some((diff) => diff.property === "gridTemplateColumns"), true);
    assert.equal(compared.propertyDiffs.some((diff) => diff.property === "backgroundImage"), true);
    assert.equal(compared.propertyDiffs.some((diff) => diff.property === "objectFit"), true);
    assert.equal(compared.propertyDiffs.some((diff) => diff.property === "hover.color"), true);
});
