...
export function normalizeExportRoutePath(value) {
...
export function readRedirectTargetFromRecord(source) {
    for (const key of [
        "redirectTo",
        "redirectUrl",
        "targetUrl",
    ...
    const link = source.link && typeof source.link === "object"
        ? source.link
        : null;
    if (link) {
...
export function readRedirectStatusFromRecord(source) {
...
export function normalizePluginExportRoutes(pluginCapture) {
    ...
    const routes = pages
        ...
        const path = typeof rawPath === "string" ? normalizeExportRoutePath(rawPath) : "";
        const rawTitle = [
            record.name,
            record.title,
            record.pageTitle,
        ...
        const collectionId = typeof record.collectionId === "string"
            ? record.collectionId
            : typeof metadata.collectionId === "string"
                ? metadata.collectionId
                : undefined;
        const templatePath = typeof record.path === "string" && record.path.trim()
            ? record.path.trim()
                ...
                return [
                    {
                        ...
                        kind: "redirect",
                        templateKind: destinationKind === "external" ? "utility" : "redirect",
        ...
        const expanded = items
            ...
            return {
                schemaVersion: EXPORT_ROUTE_SCHEMA_VERSION,
                path: normalizeExportRoutePath(route.path.replace(":slug", encodeURIComponent(slug))),
...
export function validateNormalizedExportRoutes(routes) {
    ...
    for (const route of routes) {
        ...
        if (seenPaths.has(route.path)) {
            throw new Error(`Invalid full-site route manifest: duplicate normalized path ${route.path}.`);
        }
        seenPaths.add(route.path);
        if (route.kind === "page") {
            if (route.template !== "static" && route.template !== "cms") {
                throw new Error(`Invalid full-site route manifest: page ${route.path} must declare a static or cms template.`);
            }
            continue;
        }
        if (!route.destination) {
            throw new Error(`Invalid full-site route manifest: redirect ${route.path} is missing a destination.`);
        }
        if (route.destinationKind !== "internal" &&
            route.destinationKind !== "external") {
            throw new Error(`Invalid full-site route manifest: redirect ${route.path} has an invalid destination kind.`);
        }
        if (route.destinationKind === "internal" &&
            !route.destination.startsWith("/")) {
            throw new Error(`Invalid full-site route manifest: redirect ${route.path} must point to an internal path or HTTP(S) URL.`);
        }
