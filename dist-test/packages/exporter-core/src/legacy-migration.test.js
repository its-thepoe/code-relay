...
test("migrateLegacyExportToRevision registers a legacy export as a revision and marks invalid responsive captures", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "coderelay-legacy-migration-"));
    ...
    await fs.writeFile(path.join(exportDir, "normalized-ir.json"), `${JSON.stringify({
        sourceUrl: "https://example.com",
        componentName: "LegacyExport",
    ...
    await fs.writeFile(path.join(exportDir, "raw-runtime-capture.json"), `${JSON.stringify({
    ...
    await fs.writeFile(path.join(exportDir, "raw-plugin-payload.json"), `${JSON.stringify({
        mode: "framer-plugin",
    ...
    await fs.writeFile(path.join(exportDir, "generated-validation.json"), `${JSON.stringify({
        status: "passed",
        generatedFileCount: 3,
        tsxBytes: 100,
        cssBytes: 50,
    ...
    const manifest = JSON.parse(await fs.readFile(migration.revisionManifestPath, "utf8"));
    const responsivePlan = JSON.parse(await fs.readFile(migration.responsiveRecapturePlanPath, "utf8"));
    const status = JSON.parse(await fs.readFile(migration.statusPath, "utf8"));
    const sourceArtifacts = JSON.parse(await fs.readFile(migration.sourceArtifactsPath, "utf8"));
    ...
    assert.equal(responsivePlan.migration?.legacyResponsiveViewportInvalid, true);
