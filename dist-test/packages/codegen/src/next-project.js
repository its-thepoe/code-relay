...
import { createCanonicalContentBundle, canonicalEditAreas, migrateV1ContentContractToV2, writeCanonicalSiteBundle, } from "../../content-contract/src/index.js";
...
export async function generateNextProject(input) {
    ...
    const contentContract = createCanonicalContentBundle({
        ...
        description: input.ir.exportMode === "full-site"
        ...
        content: {
            ...
            cmsCollectionNames: (input.ir.cmsCollections ?? []).map((collection) => typeof collection.name === "string" ? collection.name : typeof collection.id === "string" ? collection.id : "collection"),
        },
    ...
    await writeFile(path.join(input.projectDir, "content-contract.json"), `${JSON.stringify(contentContract, null, 2)}\n`);
    await writeCanonicalSiteBundle(migrateV1ContentContractToV2(contentContract), path.join(input.projectDir, ".coderelay"));
