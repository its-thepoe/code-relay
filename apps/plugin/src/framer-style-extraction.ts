export function extractFramerNodeStyles(
  raw: Record<string, unknown>,
): Record<string, string> {
  const font = raw.font as Record<string, unknown> | null | undefined;
...
function extractLayoutStyles(raw: Record<string, unknown>) {
  const styles: Record<string, string> = {};
...
function extractGridItemStyles(raw: Record<string, unknown>) {
  const styles: Record<string, string> = {};
...
function cssFontFamily(font: Record<string, unknown> | null | undefined) {
...
function cssBorderRadius(value: unknown): string | undefined {
...
function cssBorder(value: unknown): string | undefined {
  if (!value || typeof value !== "object") return cssString(value);
  const raw = value as Record<string, unknown>;
...
function cssOverflow(value: unknown) {
  if (typeof value === "string" && value.trim()) return value.trim();
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
...
function cssOverflowAxis(value: unknown, axis: "x" | "y") {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
  return cssString(axis === "x" ? raw.x : raw.y);
}

function cssAspectRatio(value: unknown) {
...
function cssRotation(value: unknown) {
  if (typeof value === "number" && Number.isFinite(value) && value !== 0) {
...
function cssBackgroundImage(value: unknown) {
...
function cssBackgroundPosition(value: unknown) {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
  return cssString(raw.position ?? raw.backgroundPosition);
}

function cssBackgroundSize(value: unknown) {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
  return cssString(raw.size ?? raw.backgroundSize);
}

function cssBackgroundRepeat(value: unknown) {
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
  if (typeof raw.repeat === "boolean") return raw.repeat ? "repeat" : "no-repeat";
  return cssString(raw.repeat);
}

function cssGradient(value: unknown) {
  if (typeof value === "string" && value.trim()) return value.trim();
  if (!value || typeof value !== "object") return undefined;
  const raw = value as Record<string, unknown>;
...
function mapStackDirection(value: string | undefined) {
  if (value === "horizontal") return "row";
  if (value === "vertical") return "column";
  return cssString(value);
}

function mapStackDistribution(value: string | undefined) {
...
function mapGridItemAlignment(value: string | undefined) {
  if (!value) return undefined;
  switch (value) {
    case "start":
      return "start";
...
function cssGridSpan(value: unknown) {
