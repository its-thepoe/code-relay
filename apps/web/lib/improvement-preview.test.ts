...
test("buildImprovementPreviewsForJob summarizes reuse plans from stored revision artifacts", async () => {
  const exportDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "coderelay-improvement-preview-"),
  );
  await fs.mkdir(path.join(exportDir, "source-artifacts"), {
    recursive: true,
  });
  await fs.writeFile(
    path.join(exportDir, "source-artifacts", "manifest.json"),
    JSON.stringify({
  ...
  const job: LocalExportJob = {
    ...
    artifacts: {
      ...
      responsiveRecapturePlanPath: path.join(
        exportDir,
  ...
  const responsiveness = previews.find(
    (entry) => entry.requestedFocus === "responsiveness",
  );
  const revalidate = previews.find(
    (entry) => entry.requestedFocus === "revalidate",
  );
  ...
  assert.ok(
    responsiveness?.invalidatedArtifacts.includes("runtime/responsive"),
  );
