...
export function createReportBreakdown(input: {
  ...
  capabilityReport?: JsonRecord;
  codeCompatibilityReport?: JsonRecord;
}): ReportBreakdownItem[] {
  ...
  const cmsCollectionCount =
    typeof report.cmsCollectionCount === "number" ? report.cmsCollectionCount : 0;
  const cmsCollections = Array.isArray(report.cmsCollections)
    ? (report.cmsCollections as Array<JsonRecord>)
    : [];
  ...
  const compatibilitySummary =
    ...
    typeof codeCompatibilityReport.summary === "object"
      ? (codeCompatibilityReport.summary as JsonRecord)
      : {};
  const compatibilityFileCount =
    typeof codeCompatibilityReport.fileCount === "number"
  ...
  const responsiveAverage =
    responsiveScores.length > 0
      ? Math.round(
          responsiveScores.reduce((sum, value) => sum + value, 0) /
            responsiveScores.length,
        )
      : undefined;
  const motionScore = readRounded(fidelity.motion);
  const runtimeNodesWithMotion =
    typeof motionExtraction.runtimeNodesWithMotion === "number"
      ? motionExtraction.runtimeNodesWithMotion
      : 0;
  const exportNodesWithMotion =
    typeof motionExtraction.exportNodesWithMotion === "number"
      ? motionExtraction.exportNodesWithMotion
      : 0;
  const interactionContracts = Array.isArray(validation.interactionContracts)
    ? (validation.interactionContracts as Array<JsonRecord>)
    : [];
  const failedInteractionContracts = interactionContracts.filter(
    (contract) => contract.status === "failed",
  ).length;
  const passedInteractionContracts = interactionContracts.filter(
    (contract) => contract.status === "passed",
  ).length;
  const interactionContractDetail =
    interactionContracts.length > 0
  ...
  return [
    {
      ...
      value:
        typeof validation.status === "string" ? String(validation.status) : "-",
    ...
    },
    {
      ...
      detail:
        failedRoutes === 0 ? "No empty routes detected" : `${failedRoutes} route(s) rendered empty`,
    },
    {
    ...
    },
    {
      ...
      detail:
        cmsCollectionCount > 0 ? `${cmsItemCount} items captured` : "No CMS collections detected",
    },
    {
...
function readRounded(value: unknown) {
  const number = readNumber(value);
  return number === undefined ? undefined : Math.round(number);
}

function scoreTone(value?: number): "good" | "warn" | "neutral" {
