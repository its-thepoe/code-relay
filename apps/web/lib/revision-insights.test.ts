...
} from "./revision-insights.js";

function createJob(
  id: string,
  overrides: Partial<LocalExportJob> = {},
): LocalExportJob {
  return {
    id,
    status: "completed",
...
test("collectRevisionFamily groups root, siblings, and descendants in lineage order", () => {
  ...
  const family = collectRevisionFamily(
    [grandChild, unrelated, childB, root, childA],
    grandChild.id,
  );

  assert.deepEqual(
    family.map((entry) => [entry.job.id, entry.depth, entry.isCurrent]),
    [
      [root.id, 0, false],
      [childA.id, 1, false],
      [childB.id, 1, false],
