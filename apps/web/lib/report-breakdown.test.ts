...
test("createReportBreakdown produces the required report buckets", () => {
  ...
  assert.equal(
    breakdown.find((item) => item.key === "responsive-fidelity")?.value,
    "75",
  );
...
test("createReportBreakdown prefers interaction contract coverage when available", () => {
  ...
  const interaction = breakdown.find(
    (item) => item.key === "interaction-fidelity",
  );
...
test("createReportBreakdown surfaces route and asset problems", () => {
  ...
  const assetPortability = breakdown.find(
    (item) => item.key === "asset-portability",
  );
  const portability = breakdown.find(
    (item) => item.key === "code-component-portability",
  );
  ...
  assert.equal(
    breakdown.find((item) => item.key === "interaction-fidelity")?.detail,
    "1/2 interaction contracts passed",
  );
});
