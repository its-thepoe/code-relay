...
async function main() {
  ...
  if (!args.url) {
    console.error('Usage: npm run export:test -- --url https://example.framer.website')
    process.exit(1)
  }
  ...
  console.log(`Best attempt: ${result.bestAttempt.attemptNumber} (${result.bestAttempt.strategy})`)
